...
Trust levels:
  - builtin:   Ships with the agent. Never scanned, always trusted.
  - trusted:   openai/skills and anthropics/skills only. Caution verdicts allowed.
  - community: Everything else. Any findings = blocked unless --force.

Usage:
    from nanobot.agent.tools.skills_guard import scan_skill, should_allow_install, format_scan_report
...
from nanobot.agent.tools.schema import BooleanSchema, StringSchema, IntegerSchema, ArraySchema, tool_parameters_schema
...
def should_allow_install(result: ScanResult, force: bool = False) -> Tuple[bool | None, str]:
    ...
    Returns:
        (allowed, reason) tuple - allowed is True/False/None (None means needs confirmation)
    """
...
@tool_parameters(
    tool_parameters_schema(
        skill_path=StringSchema(
            "Path to the skill directory or file to scan for security threats",
        ),
        source=StringSchema(
            "Source identifier for trust level resolution (e.g. 'openai/skills', 'community', 'agent-created')",
        ),
        force=BooleanSchema(
            description="If True, override blocked policy decisions",
            default=False,
        ),
        required=["skill_path"],
    )
)
class SkillsGuardTool(Tool):
    """Security scanner tool for externally-sourced skills.

    Scans skill directories for known-bad patterns including:
    - Data exfiltration (secret env vars, credential stores)
    - Prompt injection (jailbreaks, instruction override)
    - Destructive operations (rm -rf, disk writes)
    - Persistence mechanisms (cron, ssh keys, systemd)
    - Network exfiltration (reverse shells, tunnels)
    - Obfuscation (base64, eval, encoded strings)
    - Hardcoded secrets (API keys, tokens)

    Trust levels determine install policy:
    - builtin: Always allowed (ships with the agent)
    - trusted: Caution allowed, dangerous blocked (openai/skills, anthropics/skills)
    - community: Caution blocked, dangerous blocked
    - agent-created: All allowed, dangerous asks for confirmation
    """
    ...
    def description(self) -> str:
        return (
            "Scan a skill directory for security threats using regex-based static analysis. "
            "Detects data exfiltration, prompt injection, destructive commands, persistence mechanisms, "
            "network exfiltration, obfuscation, and hardcoded secrets. "
            "Returns a verdict (safe/caution/dangerous) and detailed findings."
        )
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        """Execute the skills guard scan."""
        try:
            path = Path(skill_path)
            if not path.exists():
                return f"Error: Path does not exist: {skill_path}"

            result = scan_skill(path, source)
            report = format_scan_report(result)

            # Add summary info
            allowed, reason = should_allow_install(result, force)
